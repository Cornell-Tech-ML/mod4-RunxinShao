...
def central_difference(f: Any, *vals: Any, arg: int = 0, epsilon: float = 1e-6) -> Any:
    ...
    *vals : n-float values $x_0 \\ldots x_{n-1}$
    ...
    """
    # Create two lists of values, one with a small positive perturbation and one with a small negative perturbation
    vals1 = [v for v in vals]
    vals2 = [v for v in vals]
    vals1[arg] = vals1[arg] + epsilon
    vals2[arg] = vals2[arg] - epsilon
    # Compute the difference in function values and divide by twice the perturbation to approximate the derivative
